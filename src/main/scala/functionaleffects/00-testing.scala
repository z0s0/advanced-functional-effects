/** TESTING
  ...
  * that seamlessly integrates with ZIO, providing access to all of ZIO's power
  * from within unit tests and full testability for all ZIO services.
  *
  ...
  */
package advancedfunctionaleffects.testing
...
/** SPECS
  ...
  * SBT test runner or the IntelliJ IDEA ZIO plug-in, you can also run your
  * tests directly from SBT or your IDE.
  */
object SimplestSpec extends ZIOSpecDefault {

  /** EXERCISE
    *
    * Using sbt or your IDE, run `SimplestSpec` by using its `main` function
    * (not the test runner).
    */
...
/** ASSERTIONS
  ...
  * powerful error messages and reporting. Assertions produce values, which
  * compose using a variety of operators.
  */
  ...
  def spec = suite("BasicAssertions") {
    ...
    test("2 + 2 == 4") {
      ...
      test("sherlock misspelling") {
        ...
        /** EXERCISE
          *
          * Examine the output of this failed test. Then fix the test so that it
          * passes.
          */
      ...
      test("multiple assertions") {
        ...
        /** EXERCISE
          ...
          */
        assertTrue(string.length == 6) && assertTrue(
    ...
    /** EXERCISE
      ...
      * `test`, as above. This test should verify that the contents of one of
      * the buildings in `buildings` contains a `needle`.
      */
  }
}
...
/** TEST ASPECTS
  ...
  * whether suites or individual tests. Test aspects are kind of like
  * annotations, except they are type-safe, non-magical, and first class values
  * that can be trasnformed and composed with other test aspects.
  *
...
object BasicTestAspects extends ZIOSpecDefault {
  ...
  def spec = suite("BasicTestAspects") {
    test("ignore") {
      ...
      /** EXERCISE
        *
        * Using `TestAspect.ignore`, add the `ignore` aspect to this test so
        * that the failure is ignored.
        */
      assertTrue(false)
    } +
      test("flaky") {
        ...
        /** EXERCISE
          *
          * Using `TestAspect.flaky`, mark this test as flaky so that it will
          * pass so long as it sometimes succeeds.
          */
        for {
      ...
      test("nonFlaky") {
        ...
        /** EXERCISE
          *
          * Using `TestAspect.nonFlaky`, mark this test as non-flaky so that
          * ZERO failures are permitted.
          */
        for {
...
/** TEST FIXTURES
  *
  * ZIO can execute arbitrary logic before, after, or before and after tests
  * individually, or all tests in a suite. This ability is sometimes used for
  * "test fixtures", which allow developers to perform custom setup / tear down
  * operations required for running tests.
  */
object TestFixtures extends ZIOSpecDefault {
  ...
  def spec = suite("TestFixtures") {
    ...
    test("before") {
      ...
      /** EXERCISE
        *
        * Using `TestAspect.after`, ensure the message `done with after` is
        * printed to the console using `ZIO.debug`.
        */
      test("after") {
        for {
      ...
      /** EXERCISE
        *
        * Using `TestAspect.around`, ensure the `aroundRef` is incremented
        * before and decremented after the test.
        */
      test("around") {
        for {
...
/** TEST SERVICES
  *
  * By default, ZIO tests use test versions of all the standard services baked
  * into ZIO, including Random, Clock, System, and Console. These allow you to
  * programmatically control the services, such as adjusting time, setting up
  * fake environment variables, or inspecting console output or providing
  * console input.
  */
  ...
  def spec =
    suite("TestServices") {
      ...
      test("TestClock") {
        ...
        /** EXERCISE
          *
          * Using `TestConsole.feedLines`, feed a name into the console such
          * that the following test passes.
          */
        test("TestConsole") {
          for {
          ...
          for {
            ...
            result <-
              if (guess == number.toString)
                Console.printLine("Good job!").as(true)
        ...
        /** EXERCISE
          ...
          * service, rather than one of the test services baked into ZIO Test. A
          * useful function for doing this is `Live.live`, which will ensure the
          * provided effect runs using the live services.
          */
        test("Live") {
          for {
...
object IntegrationSystem extends ZIOSpecDefault {
  ...
  /** EXERCISE
    *
    * Explore jvmOnly, windows, linux, ifEnv, and other test aspects that are
    * useful for running platform-specific or integration / system tests.
    */
...
/** CUSTOM LAYERS
  *
  * The code you are testing may use its own layers, to provide access to other
  * services required by your application. This is especially true for business
  * logic, which may be assembled from high-level layers that allow expressing
  * business logic in a direct style.
  *
  * ZIO Test allows you to provide custom layers in a variety of ways to your
  * tests.
  */
    ...
    suite("CustomLayers") {
      test("provideCustomLayer") {
        ...
        /** EXERCISE
          ...
          * associated with the id. Then check the age is 42. To make the test
          * compile, you will have to `provideCustomLayer` on the test. Finally,
          * to make the test pass, you will have to create test data matches
        ...
        /** EXERCISE
          ...
          * across both of the following (sequentially executed) tests. Then add
          * a user in the first test that is then retrieved in the second.
          */
        suite("shared layer") {
          test("adding a user") {
            assertTrue(false)
...
/** GRADUATION PROJECT
  ...
  * implementation that allows simulating failures and successes, and which
  * captures sent emails for purposes of testing. Finally, create a layer for
  * the test email service and use it in a test.
  */
