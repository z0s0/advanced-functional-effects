  ...
  def spec =
    suite("RunningStreams") {
      ...
      test("runHead") {
        ...
        test("foldZIO") {
          val expected =
            List("What is your name?" -> "Sherlock Holmes", "What is your age?" -> "42")

          val questions = ZStream.fromIterable(expected.map(_._1))

          for {
            _ <- TestConsole.feedLines(expected.map(_._2): _*)
            map <- questions
                    .mapZIO(question => ZIO.succeed(question) <*> Console.readLine)
                    .run(ZSink.collectAll)
                    .map(_.toMap)
          } yield assertTrue(map == expected.toMap[String, String])
        }
    }
}
  ...
  def spec =
    suite("AdvancedOperators") {
      ...
      test("flatMap") {
        ...
        // ZStream(1 ,1, 1) ++ ZStream(2, 2, 2) ++ ZStream(3, 3, 3)
